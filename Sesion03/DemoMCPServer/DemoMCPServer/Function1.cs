...
public class Function
{
    ...
    [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "mcp")] HttpRequestData req)
    {
        ...
        _logger.LogInformation("Method: {Method}, Id: {Id}", method, id);
    ...
    private async Task<HttpResponseData> HandleToolsList(HttpRequestData req, object? id)
    {
        ...
        .Select(method =>
        {
            ...
            var parameters = method.GetParameters().Select(p => new
            {
                name = p.Name,
                description =
            p.GetCustomAttribute<DescriptionAttribute>()?.Description ?? p.Name, type = MapType(p.ParameterType)}).ToArray();
    ...
    private async Task<HttpResponseData> HandleToolsCall(HttpRequestData
    req, JsonElement root, object? id)
    {
        if (!root.TryGetProperty("params", out var @params)) return await ErrorResponse(req, -32602, "Invalid params", id);
        ...
        try
        {
            var method = typeof(DemoTools).GetMethod(toolName ?? "") ?? throw new ArgumentException($"Tool not found: { toolName }");
            var parameters = method.GetParameters();
            var invokeArgs = parameters.Select(p =>
                {
                    if (!hasArgs) return Type.Missing;
                    if (!arguments.TryGetProperty(p.Name!, out var val))
                        return Type.Missing;
                    return JsonSerializer.Deserialize(val.GetRawText(), p.ParameterType, JsonOpts)!;
                }
            ).ToArray();
            var result = method.Invoke(_tools, invokeArgs);
            _logger.LogInformation("Tool {Tool} invoked, result type: { Type} ", toolName, result?.GetType().Name ?? "null");
            
            var text = result == null ? string.Empty :
        ...
        {
            return await ErrorResponse(req, -3200,
            tie.InnerException?.Message ?? tie.Message, id);
        }
        catch (Exception ex)
        {
            return await ErrorResponse(req, -3200, ex.Message, id);
        }
    }

    private static async Task<HttpResponseData> ErrorResponse
    (HttpRequestData req, int code, string message, object? id)
    {
    ...
    private static string MapType(Type t) => t == typeof(double) || t == typeof(int) || t == typeof(float) ?"number" :t == typeof(bool) ? "boolean" : "string";
